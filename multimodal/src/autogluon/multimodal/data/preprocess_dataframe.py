...
class MultiModalFeaturePreprocessor(TransformerMixin, BaseEstimator):
    ...
    def transform_label(
        ...
        if self.label_type == CATEGORICAL:
            if self._fewshot:
                self._label_generator.fit(y_df)
    ...
    def transform_prediction(
        ...
        fewshot_label_list: Optional[np.ndarray] = None,
    ) -> NDArray[(Any,), Any]:
        """
        ...
        fewshot_label_list
            The few_shot label list to transform few_Shot label into true label.
        ...
        if self.label_type == CATEGORICAL:
            ...
            if fewshot_label_list is not None:
                for idx, per_pred in enumerate(y_pred):
                    y_pred[idx] = fewshot_label_list[idx, per_pred]
